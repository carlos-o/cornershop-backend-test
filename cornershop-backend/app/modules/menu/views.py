...
from app.modules.utils.services import is_valid_uuid
...
class MenuViewSet(ModelViewSet):
	...
	def retrieve(self, request, *args, **kwargs):
		...
		if is_valid_uuid(kwargs.get('pk')):
			try:
				instance = menu_models.Menu.objects.get(uuid=kwargs.get('pk'))
			except Exception as e:
				logger.error(f"Menu does not exist {e}")
				return Response(ResponseDetail().not_found_detail(), status=status.HTTP_404_NOT_FOUND)
	...
	@action(
		methods=['PUT', 'DELETE'], url_path='option/(?P<option_id>[0-9]+)', detail=True,
		permission_classes=(permissions.IsAuthenticated, permissions.IsAdminUser)
	)
	def delete_update_option(self, request, pk=None, option_id=None):
		instance = self.get_object()
		if request.method == 'DELETE':
			...
			try:
				...
				return Response(
					...
					status=status.HTTP_500_INTERNAL_SERVER_ERROR)
			return Response(
				ResponseDetail().success_detail(message="options has been delete successfully"),
				status=status.HTTP_200_OK)
